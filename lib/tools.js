...
import { ENGRAM_LAYERS, isVisible, isSuperseded } from './engram/store.js';
const TEXT_OUTPUT = {
    schema: { type: 'string' },
    render: (_args, value) => [{ type: 'text', text: String(value) }],
};
const KINDS = ['fact', 'decision', 'event', 'note'];
/** 查看者视角（跨会话可见性边界）：从工具执行上下文解析。 */
function viewerOf(exec) {
...
function resolveNode(relay, ref) {
 ...
 */
async function recommendLinks(relay, text, excludeId) {
 ...
 */
async function weaveLinks(relay, node, detailed) {
    ...
    for (const [id] of ranked.slice(0, 2)) {
        ...
        if (!target || target.id === node.id)
            continue;
        if (target.links.includes(node.title))
            continue;
        relay.store.update(target.id, { links: [...target.links, node.title] });
        if (!newLinks.includes(target.title))
            newLinks.push(target.title);
        woven++;
    }
    if (woven > 0)
        relay.store.update(node.id, { links: newLinks });
    return woven;
}
/** 入口行渲染（[[标题]][层] 摘要 + **因果/链接邻接**——渐进披露入口层
 *  就给导航信息：↑因/↓果（因果链）→ 关联（双向链接），模型看到即可
 *  决定顺着哪条边探究（engram_open 展开正文），不用盲目逐个 open。 */
function entryLine(e, store, graph) {
...
function neighborsOf(e, store, graph) {
    const parts = [];
    const nodeOf = (id) => store.get(id);
    const titlesOf = (ids, max) => {
        const withImp = ids
            .map((id) => store.get(id))
            .filter((n) => !!n)
    ...
    if (causes.length > 0)
        parts.push(`↑因:${render(causes)}${(e.causes?.length ?? 0) > 4 ? '…' : ''}`);
    if (effects.length > 0)
        parts.push(`↓果:${render(effects)}${(e.effects?.length ?? 0) > 4 ? '…' : ''}`);
    if (links.length > 0)
        parts.push(`→:${render(links)}`);
...
export function installEngramTools(ctx, relay) {
    const disposers = [];
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            const causes = (Array.isArray(args.causes) ? args.causes.map(String) : [])
                .map(resolveRef).filter((x) => x !== null);
            ...
            if (dedupe && dedupe.id !== e.id) {
                ...
                for (const c of old.causes ?? []) {
                    ...
                    if (n)
                        relay.store.update(c, { effects: [...n.effects.filter((x) => x !== old.id), e.id] });
                }
                for (const ef of old.effects ?? []) {
                    const n = relay.store.get(ef);
                    if (n)
                        relay.store.update(ef, { causes: [...n.causes.filter((x) => x !== old.id), e.id] });
                }
                relay.graph.rebuild();
            ...
            for (const causeId of causes) {
                ...
                if (c) {
                    ...
                    if (!c.effects.includes(e.id))
                        relay.store.update(c.id, { effects: [...c.effects, e.id] });
                }
            }
            for (const effectId of effects) {
                ...
                if (t) {
                    ...
                    if (!t.causes.includes(e.id))
                        relay.store.update(t.id, { causes: [...t.causes, e.id] });
                }
            }
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!node)
                return `未找到节点 [[${titleRef}]]（可 engram_search 检索）`;
            ...
            const conflictNote = conflictIds.length > 0
                ...
                : '';
            // 可见性：只能展开自己可见层的节点
            if (!isVisible(node, viewerOf(exec)))
                return `无权展开 [[${node.title}]]（${node.layer} 层对当前会话不可见）`;
            ...
            const linked = relay.store.getMany(node.links.map((t) => relay.store.byTitle(t)?.id ?? '').filter(Boolean));
            ...
            if (causes.length > 0)
                ...
                parts.push('\n**前因**（因果 ↑）：（无）');
            if (effects.length > 0)
                parts.push(`**后果**（因果 ↓）：${effects.map((c) => `[[${c.title}]]`).join('、')}`);
            else
                parts.push('**后果**（因果 ↓）：（无）');
            if (linked.length > 0)
                parts.push(`**关联**（双向链接）：${linked.map((c) => `[[${c.title}]]`).join('、')}`);
            else
                parts.push('**关联**（双向链接）：（无）');
            if (deps.length > 0)
                parts.push(`**依赖/引用**：${deps.map((c) => `[[${c.title}]]`).join('、')}`);
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            let nodes = relay.store.query({
                layer: layer && ENGRAM_LAYERS.includes(layer) ? layer : undefined,
            ...
            if (q)
                nodes = nodes.filter((e) => e.title.toLowerCase().includes(q) || e.summary.toLowerCase().includes(q));
            ...
            for (const [title, group] of byTitle) {
                ...
                const parts = [];
                const renderTitles = (ts) => ts
                    .map((t) => {
                    const target = relay.store.byTitle(t);
                    return `[[${t}]]${target && isSuperseded(target) ? '（已废止）' : ''}`;
                })
                    .join(' ');
                const titlesOf = (ids, max) => ids
                ...
                const links = (current.links ?? []).slice(0, 3)
                    .map((l) => String(l).replace(/^\[\[|\]\]$/g, '').trim());
                if (causes.length > 0)
                    parts.push(`↑因:${renderTitles(causes)}${(current.causes?.length ?? 0) > 4 ? '…' : ''}`);
                if (effects.length > 0)
                    parts.push(`↓果:${renderTitles(effects)}${(current.effects?.length ?? 0) > 4 ? '…' : ''}`);
                if (links.length > 0)
                    parts.push(`→:${renderTitles(links)}`);
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!from)
                return `未找到源节点 [[${args.from}]]`;
            if (!to)
                return `未找到目标节点 [[${args.to}]]`;
            if (from.id === to.id)
                return '错误：不能连接节点自身';
            const viewer = viewerOf(exec);
            ...
            const kind = String(args.kind ?? 'causes');
            ...
            if (kind === 'causes') {
                if (!from.effects.includes(to.id))
                    relay.store.update(from.id, { effects: [...from.effects, to.id] });
                if (!to.causes.includes(from.id))
                    relay.store.update(to.id, { causes: [...to.causes, from.id] });
            }
            else {
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!node)
                return `未找到节点 [[${args.ref}]]`;
            if (!isVisible(node, viewerOf(exec)))
                return `无权修正 [[${node.title}]]（${node.layer} 层对当前会话不可见）`;
            const patch = {};
            if (args.title !== undefined)
                patch.title = String(args.title);
            if (args.summary !== undefined)
                patch.summary = String(args.summary);
            if (args.content !== undefined)
                patch.content = String(args.content);
            if (args.links !== undefined)
                patch.links = args.links.map(String);
            if (args.importance !== undefined)
                patch.importance = Number(args.importance);
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!node)
                return `未找到节点 [[${args.ref}]]`;
            if (!isVisible(node, viewerOf(exec)))
                return `无权删除 [[${node.title}]]（${node.layer} 层对当前会话不可见）`;
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!node)
                return `未找到节点 [[${args.ref}]]`;
            if (!isVisible(node, viewer))
                return `无权提升 [[${node.title}]]（${node.layer} 层对当前会话不可见）`;
            const target = String(args.layer);
            ...
            if (node.layer === 'global')
                return `[[${node.title}]] 已是 global 层（最高），无需提升`;
            if (node.layer === target)
                return `[[${node.title}]] 已在 ${target} 层`;
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        isConcurrencySafe: () => true,
        execute: async (args, exec) => {
            const isIsolated = (e) => !isSuperseded(e) && e.status !== 'pending'
