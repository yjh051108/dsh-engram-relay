...
import { ENGRAM_LAYERS, isVisible } from './engram/store.js';
const TEXT_OUTPUT = {
    schema: { type: 'string' },
...
export function installEngramTools(ctx, relay) {
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (hit.engrams.length === 0)
                return `（无命中，reason=${hit.reason}——诚实边界：图谱无相关记忆）`;
            ...
            const vmarks = hit.verify;
            const markOf = (e) => {
                const m = vmarks?.[e.id];
                if (!m)
                    return '';
                if (m.status === 'anchored')
                    return ' ✓已锚定';
                if (m.status === 'partial')
                    return ' ~部分锚定';
            ...
            return hit.engrams.map((e) => entryLine(e) + markOf(e) + (e._tag ?? '')).join('\n');
        },
    })));
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (!node)
                return `未找到节点 [[${args.title}]]（可 engram_search 检索）`;
            // 可见性：只能展开自己可见层的节点
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args) => {
            ...
            if (!m)
                return '（灵枢融合未启用：lingshuVerifyUrl 未配置）';
            if (m.status === 'error')
                return `（灵枢验证不可用：${m.note ?? ''}）`;
            if (m.status === 'anchored')
                return `✓已锚定（${m.note ?? '图谱确认'}）：该主张与灵枢知识图谱一致，可溯源`;
            if (m.status === 'partial')
                return `~部分锚定：${m.note ?? '部分主张命中图谱'}`;
    ...
    disposers.push(ctx.tools.register(defineTool({
        ...
        execute: async (args) => {
            ...
            if (!condition)
                return '（空条件）';
            const r = (await relay.lingshuRespond(condition, Number(args.limit ?? 3)));
            if (r.error)
                return `（灵枢出招不可用：${r.error}）`;
