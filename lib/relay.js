...
import { ENGRAM_LAYERS, isSuperseded } from './engram/store.js';
...
export class EngramRelay {
                    ...
                    try {
                        ...
                        if (injection) {
                            ...
                            if (Array.isArray(messages)) {
                                ...
                                messages.push({ role: 'system', content: [{ type: 'text', text: injection }] });
                            }
                        }
                    }
                    catch (error) {
    ...
    async status() {
        ...
        return {
            ...
            isolatedCount: this.store.all().filter((e) => !isSuperseded(e) && e.status !== 'pending'
                && (e.causes?.length ?? 0) === 0 && (e.effects?.length ?? 0) === 0 && (e.links?.length ?? 0) === 0).length,
