...
export class EngramRelay {
    ...
    async lingshuAutoVerify(text) {
        ...
        try {
            ...
            if (j.startsWith('采纳'))
                return { status: 'anchored', note: `${bestName}（D_norm=${dnorm}）` };
        ...
        this.disposers.push(this.ctx.on('llm/stream', (options, next) => {
            if (this.config.enabled) {
                ...
                const wakeP = this.wake.maybeWake(sessionId, options, { cwd: this.currentCwd ?? undefined, turn: this.lastTurnAt }).then((h) => {
                    if (h) {
                        this.debugLog(`wake ${h.reason}${h.pressure ? ' [pressure]' : ''} items=${h.engrams.length}`);
                    }
                    return h;
                }).catch((error) => {
    ...
    async maybeDistill() {
        ...
        for (const item of parsed) {
            ...
            {
                const _tgrams = new Set();
                for (const _seg of (String(item.title).match(/[\u4e00-\u9fff]+/g) ?? [])) {
                    for (let _i = 0; _i < _seg.length - 1; _i++)
                        _tgrams.add(_seg.slice(_i, _i + 2));
                }
                let _dup = false;
                if (_tgrams.size > 0) {
