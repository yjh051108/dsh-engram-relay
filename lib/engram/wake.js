 ...
 * 语义 + 因果）。
 */
import { isVisible, dormantOf, isLesson } from './store.js';
export class EngramWakeEngine {
        ...
        if (opts.auto && lessonMin > 0) {
            ...
            for (const [id] of lessonRanked) {
                ...
                if (!e)
                    continue;
                const cost = estimateTokens(e.title) + estimateTokens(e.summary);
                if (lTokens + cost > lessonBudget && lTokens > 0)
                    break;
                picked.push(e);
                lTokens += cost;
                tokens += cost;
    ...
    renderInjection(budgetTokens) {
        ...
        engrams.forEach((e) => {
            ...
            if (isL && lessonTokens >= lessonBudget)
                return;
            if (!isL && tokens >= budgetTokens)
                return;
            let line;
            if (isL) {
            ...
            if (isL) {
                if (lessonTokens + cost > lessonBudget)
                    return;
                lessonTokens += cost;
            }
            else {
