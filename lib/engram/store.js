...
export const ENGRAM_LAYERS = ['global', 'project'];
/**
 * 可见性判定（v0.4：全可见——项目不隔离，关联即桥）。
 * 保留函数签名（wake/tools/图谱 API 调用点不动），恒返回 true。
 */
export function isVisible(_e, _viewer) {
    return true;
}
/** 默认标签（v0.4 迁移/兜底）：layer + projectId → 命名空间标签。 */
export function defaultTags(layer, projectId) {
    if (layer === 'global' || projectId === null || projectId === '')
        return ['全局'];
 ...
 */
export function dormantOf(e, now = Date.now()) {
    if (e.hits >= 3 && e.kind !== 'event')
        return false;
...
export function isSuperseded(e) {
...
export class EngramStore {
        ...
        for (const src of sources) {
            ...
            for (const line of cleaned.split('\n')) {
                ...
                try {
                    ...
                    e.layer = e.layer === 'session' ? 'project' : (e.layer ?? 'project');
    ...
    add(input) {
        ...
        if (node.title)
            this.indexTitle(node.title, node.id);
        this.persist();
        return node;
    }
    /** 标题索引登记（多值聚合，去重）。 */
     ...
     */
    upsertSnapshot(cwd, turn, sessionId = null) {
        if (!cwd)
            return null;
    ...
    lookupHash(result, text = '', limit = 8) {
        ...
        if (tokenIds.size > 0)
            iters.push(tokenIds.values());
        const budget = Math.max(limit, 64);
        let round = 0;
        outer: while (hits.length < budget && round < 64) {
            let allDone = true;
    ...
    stateCounts() {
        const counts = { episodic: 0, semantic: 0, dormant: 0 };
        for (const e of this.byId.values()) {
