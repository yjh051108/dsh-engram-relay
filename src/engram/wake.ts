...
import { EngramStore, isVisible, dormantOf, isLesson, type EngramLayer, type EngramNode } from './store.js'
...
export class EngramWakeEngine {
  ...
  async query(query: string, limit: number, viewer: WakeViewer = {}, opts: { auto?: boolean } = {}): Promise<WakeHit> {
    ...
    // ═══ 教训通道（v0.6）═══
    // 主注入按高分 top-K 走（auto 阈值 ≈0.50 之上）；教训类节点（tags 含
    // 「教训:」）用更低阈值（lessonMinScore，默认 0.42）独立占一席——同类
    // 操作时「踩过的坑」即使语义分卡在阈值下也要提醒（这正是教训最有价值
    // 的场景：低相关度但同类操作）。独立预算 lessonBudgetTokens，不挤占
    // 普通记忆；0 = 关闭通道。
    const lessonMin = this.config.lessonMinScore ?? 0.42
    if (opts.auto && lessonMin > 0) {
      const lessonBudget = this.config.lessonBudgetTokens ?? 60
      let lTokens = 0
      const lessonRanked = [...activated.entries()]
        .filter(([id]) => hitIds.has(id) && relevantIds.has(id))
        .filter(([id]) => (activated.get(id) ?? 0) >= lessonMin)
        .filter(([id]) => isLesson(this.store.get(id)))
        .sort((a, b) => fusionScore(b[0], b[1]) - fusionScore(a[0], a[1]))
      for (const [id] of lessonRanked) {
        if (picked.some((p) => p.id === id)) continue
        const e = this.store.get(id)
        if (!e) continue
        const cost = estimateTokens(e.title) + estimateTokens(e.summary)
        if (lTokens + cost > lessonBudget && lTokens > 0) break
        picked.push(e)
        lTokens += cost
        tokens += cost
        this.store.touch(id)
        break // 教训席位 1 条（极克制：提醒而非淹没）
      }
    }
  ...
  renderInjection(budgetTokens: number): string {
    ...
    engrams.forEach((e) => {
      const isL = isLesson(e)
      if (isL && lessonTokens >= lessonBudget) return
      if (!isL && tokens >= budgetTokens) return
      let line: string
      if (isL) {
        // 教训行：⚠️ 标记 + 教训分类 tag——模型识别「这是踩过的坑」，同类
        // 操作时优先警惕（渲染最简格式：提醒快、占位小）。
        const lessonTag = (e.tags ?? []).find((t) => typeof t === 'string' && t.startsWith('教训:'))
        line = `- ⚠️教训[[${e.title}]][${e.layer}]${lessonTag ? `(${lessonTag})` : ''}: ${e.summary.slice(0, 100)}`
      } else if (e.state === 'semantic') {
      ...
      if (isL) {
        if (lessonTokens + cost > lessonBudget) return
        lessonTokens += cost
      } else {
