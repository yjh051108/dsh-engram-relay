...
export class EngramWakeEngine {
  ...
  constructor(
    ...
    private verifier: ((node: EngramNode) => Promise<VerifyMark | null>) | null = null,
    /** 融合：浅思维钩子（对查询+唤醒结果返回浅思维文本段）；null = 关闭。 */
    private knowledgeSource: ((query: string, hit: { engrams: EngramNode[] }) => Promise<string | null>) | null = null,
  ) {
  ...
  async maybeWake(sessionId: string, _options: GenerateOptions, viewer: WakeViewer = {}): Promise<WakeHit> {
    ...
    const emptyHit = async (reason: string): Promise<WakeHit> => {
      await this.pullKnowledge(query, { engrams: [] })
      const h: WakeHit = { engrams: [], reason, injectedTokens: 0 }
      this.lastInjection = h
      return h
    }
    const emptyHitSync = (reason: string): WakeHit => {
      const h: WakeHit = { engrams: [], reason, injectedTokens: 0 }
      this.lastInjection = h
      return h
    }
    if (this.store.count() === 0) return emptyHit('empty-store')

    if (query.trim() === '') return emptyHit('no-query')
  ...
  private async pullKnowledge(query: string, hit: { engrams: EngramNode[] }): Promise<void> {
  ...
  async query(query: string, limit: number, viewer: WakeViewer = {}, opts: { auto?: boolean } = {}): Promise<WakeHit> {
    ...
    const seenT = new Map<string, EngramNode>()
    for (const e of hit.engrams) {
      const prev = seenT.get(e.title)
      if (!prev || (e.hits ?? 0) > (prev.hits ?? 0)) seenT.set(e.title, e)
    }
  ...
  renderInjection(budgetTokens: number): string {
    ...
    const HEADER = '（统一大脑自动注入：记忆入口 = 相关往事；浅思维 = 灵枢校准（条件/验证/边界）——「图谱外」可用 engram_verify/engram_respond 求助深挖，查不到会自动补卡。细节用 engram_open 展开）'
    const lines: string[] = []
    ...
    engrams.forEach((e, idx) => {
      ...
      if (v) {
        if (v.status === 'anchored') line += ' ✓已锚定'
        else if (v.status === 'partial') line += ' ~部分锚定'
        else if (v.status === 'unverified') line += ' ?图谱外'
      }
