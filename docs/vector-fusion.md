...
| # | 场景 | 问题 | 需要的机制 |
|---|---|---|---|
| 1 | "缓存命中率怎么又降了"（问题重现） | 时序衰减把真正相关的旧诊断记忆压死 | 时序与相关性的**双路注入**（最新状态 + 历史关键诊断） |
| 2 | 架构改了 20 版 | 多条"当前真理"，检索命中过时版本 | 同主题写入 = **修订 + 废止旧版**（版本链） |
| 3 | 两个会话同 cwd 不同主题 | "缓存"一词多义，project 层语境错配 | 分层下探到**会话主题**（不止 cwd） |
| 4 | 一年蒸馏 500 条"bug 修复" | 一次性事件洪水淹没可复用知识 | 蒸馏**价值分流**（知识→长期，事件→会话归档） |
| 5 | 记忆里的数字过期（274→252） | 事实型记忆悄悄腐朽，无人修订 | 新事实写入时**自动修订旧数字**（冲突检测） |
| 6 | "继续昨天的工作" | 检索给不了"工作快照"，只有零散记忆 | 会话收尾的**工作快照**（进行中状态聚合） |

这六类场景是"演化治理"的落地验收标准——治理机制做好后，每个场景都有对应解法。
